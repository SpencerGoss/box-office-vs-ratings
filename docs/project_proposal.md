...
how much audiences like it. One might assume these two measures align, but that is not
always the case. For those looking to create a film, or interested in understanding the
movie business more broadly, comparing these metrics against each other can surface
valuable insights.

## Problem Statement

People making decisions in entertainment, as well as the general audience, may lack a
clear understanding of why certain movies are being made and why one film genre may be
more beneficial to target than another. This project aims to bridge that gap and understand
...
- **Automation:** a single re-runnable ETL pipeline (`etl_pipeline.py`) covering extract,
  transform, validate, and load, that is idempotent (safe to re-run on demand; `ON CONFLICT`
...
2. **Power BI → Dash + Plotly.** The original plan loaded a CSV into Power BI. The final
   ...
   head-to-head comparisons) that a static report could not match, while keeping the
